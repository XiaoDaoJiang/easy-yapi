...
class EasyApiSettingsConfigurableTest : EasyApiLightCodeInsightFixtureTestCase() {
    ...
    fun testApplySavesGeneralOnceAndPublishesOneTypedChange() {
        ...
        // EasyYapi has 2 saves: one from the YApi channel's self-contained
        // YapiSettingsPanel (which persists switchNotice via SettingBinder.update),
        // plus the main apply() save. easy-api expects 1 (no YApi channel).
        assertEquals("GeneralSettings should be saved exactly twice (main + YApi panel)", 2, recordingBinder.generalSaveCount)
